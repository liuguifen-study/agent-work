...
test('DeepSeek Harness uses its official headless profile and is not treated as a live inbox agent', () => {
  assert.equal(isAgentProvider('deepseek'), true);
  assert.equal(inferAgentProvider('dsh --profile headless'), 'deepseek');
  const preset = providerPreset('deepseek');
  assert.equal(preset.defaultCommand, 'dsh --profile headless');
  assert.equal(preset.supportsModel, false);
  assert.equal(preset.positionalInitialPrompt, true);
  assert.equal(preset.canReceiveInbox, false);
  assert.equal(buildSpawnCommand(autoConfig, undefined, 'deepseek'), 'dsh --profile headless');
});
...
test('model picker options stay provider-specific', () => {
  ...
  assert.deepEqual(modelsForProvider('deepseek'), []);
});
