...
test('DeepSeek Harness is a recognized, headless execution provider', () => {
  assert.ok(ap.isAgentProvider('deepseek'));
  assert.strictEqual(ap.inferAgentProvider('dsh --profile headless'), 'deepseek');
  const p = ap.providerPreset('deepseek');
  assert.strictEqual(p.defaultCommand, 'dsh --profile headless');
  assert.strictEqual(p.positionalInitialPrompt, true);
  assert.strictEqual(p.canReceiveInbox, false);
  assert.strictEqual(p.installCommand, 'npm install -g @deepseek-ai/dsh');
});
