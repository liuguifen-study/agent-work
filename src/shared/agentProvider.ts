...
export const AGENT_PROVIDER_PRESETS: AgentProviderPreset[] = [
  ...
  {
    // DeepSeek Harness (`dsh`) has a web UI and a one-shot, persisted headless
    // profile. The latter maps cleanly onto a terminal worker: the hive seed is
    // its job argument, it prints a final answer, then exits. This is deliberately
    // not inbox-capable: unlike an interactive TUI there is no safe prompt to wake
    // after the run finishes. DSH owns model selection and approval policy in its
    // own profile settings, so the harness does not invent a --model/auto flag.
    id: 'deepseek',
    label: 'DeepSeek Harness',
    defaultCommand: 'dsh --profile headless',
    commandGroups: [],
    autoModeFlag: '',
    autoFlag: '',
    supportsModel: false,
...
export function inferAgentProvider(command: string | undefined, explicit?: unknown): AgentProvider {
  ...
  if (bin === 'dsh' || bin === 'deepseek-harness') return 'deepseek';
