...
export class HiveManager {
    ...
    for (const id of readdirSync(agentsDir)) {
      ...
      for (const f of readdirSync(outbox)) {
        ...
        try {
          ...
          // Malformed/unroutable file — quarantine so we don't spin on it, then
          // make the failure VISIBLE. A bad envelope used to vanish into
          // outbox/.sent/bad-*.json with no log line and no renderer event, so
          // a broken sender (schema drift, half-written write) failed silently
          // until its mail stopped arriving.
          const quarantined = `bad-${f}`;
          try { renameSync(full, join(outbox, '.sent', quarantined)); } catch { /* noop */ }
          this.emitQuarantine(id, quarantined, e);
        }
      }
    }
  ...
  /** Record a structured quarantine event (log.jsonl + renderer push) for an
   *  outbox envelope that became `bad-*` under `outbox/.sent`. METADATA-ONLY by
   *  design — it preserves the sender, the quarantined filename and a redacted
   *  one-line reason, and NEVER the envelope contents, so a malformed file that
   *  half-captured a secret (pasted key, token in a body) cannot leak it into
   *  the append-only log or across IPC. Best-effort like appendLog: a failed
   *  log/emit must never break routing. */
  private emitQuarantine(agentId: string, filename: string, error: unknown): void {
    const payload = { agentId, file: filename, reason: this.quarantineReason(error) };
    this.appendLog({ kind: 'mail-quarantine', ...payload });
    this.emit?.('hive:mailQuarantine', payload);
  }

  /** Reduce the error behind a quarantined envelope to a short, safe reason
   *  line. JSON.parse failures dominate (Node names only a position), but fs
   *  errors from the route path can land here too — whatever the source, the
   *  text runs through the same main-side redaction battery as message bodies
   *  and is length-capped, so a reason can never smuggle envelope content or a
   *  secret-shaped fragment into log.jsonl or across IPC. */
  private quarantineReason(error: unknown): string {
