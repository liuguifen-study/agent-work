...
export interface RealtimeSpawnSpec {
  ...
  cwd: string;
  command: string;
  /** Fixed CLI arguments for a built-in provider. Kept separate from command so
   * node-pty receives an executable rather than a shell-like command string. */
  args?: string[];
  provider?: string;
...
const PROVIDER_COMMAND: Record<string, { command: string; args?: string[] }> = {
  claude: { command: 'claude' }, codex: { command: 'codex' }, antigravity: { command: 'antigravity' }, gemini: { command: 'gemini' },
  opencode: { command: 'opencode' }, crush: { command: 'crush' }, pi: { command: 'pi' }, qwen: { command: 'qwen' }, copilot: { command: 'copilot' },
  cursor: { command: 'cursor-agent' }, deepseek: { command: 'dsh', args: ['--profile', 'headless'] }
};
...
function proposeDestructive(deps: RealtimeActionDeps, verb: string, a: Record<string, unknown>): ActionResult {
  ...
  if (verb === 'spawn') {
    ...
    const requestedCommand = str(a.command);
    const engine = PROVIDER_COMMAND[provider];
    const command = requestedCommand || engine?.command || 'claude';
    const id = `${slug(name)}-${shortId()}`;
    const spec2: RealtimeSpawnSpec = {
      id, cwd, command, provider,
      // A human-supplied command owns its complete argv; fixed profile args are
      // only attached for a built-in default such as DeepSeek Harness headless.
      args: requestedCommand ? undefined : engine?.args,
