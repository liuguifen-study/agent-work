...
function FloorTab({ seed }: { seed: { text: string; seq: number } }) {
  ...
  const teammates = agents.filter((a) => !a.isGod && !a.isAssistant);
  const activeTeammates = teammates.filter((a) => a.status === 'working' || a.status === 'thinking');
  const waitingTeammates = teammates.filter((a) => a.status === 'blocked' || a.status === 'waiting' || a.onHold);
  const queuedMessages = Object.values(messageQueues).reduce((count, queue) => count + queue.length, 0);

  return (
    <Scroll>
      <CoordinationPulse
        active={activeTeammates}
        waiting={waitingTeammates}
...
/** A quiet, action-first view of the whole team. The terminal and task board
 * stay available below, but this prevents operators from having to scan a long
 * roster just to answer "who is free, who needs help, and is anything waiting?" */
function CoordinationPulse({
  ...
  const { t } = useTranslation();
  const cards = [
    { key: 'active', label: t('commandCenter.pulseActive'), value: active.length, tone: 'var(--cth-mint)', agents: active },
    { key: 'waiting', label: t('commandCenter.pulseAttention'), value: waiting.length, tone: 'var(--cth-lemon)', agents: waiting },
    { key: 'queued', label: t('commandCenter.pulseQueued'), value: queuedMessages, tone: 'var(--cth-sky)', agents: [] as Agent[] }
  ];
  return (
    <section aria-label={t('commandCenter.pulseTitle')} style={{
      padding: '10px 12px', background: 'var(--cth-ink-900)', color: 'var(--cth-paper-100)',
      boxShadow: 'inset 0 0 0 1px var(--cth-ink-900)'
    }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
        <span style={{ fontFamily: 'var(--cth-font-display)', fontSize: 9, lineHeight: '13px' }}>{t('commandCenter.pulseTitle')}</span>
        <span style={{ fontSize: 12, color: 'var(--cth-cream-200)' }}>{t('commandCenter.pulseHint')}</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 6 }}>
        {cards.map((card) => (
          <div key={card.key} style={{ padding: '7px 8px', background: 'var(--cth-paper-100)', color: 'var(--cth-ink-900)', boxShadow: `inset 3px 0 0 ${card.tone}` }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
              <strong style={{ fontFamily: 'var(--cth-font-mono)', fontSize: 18, lineHeight: '20px' }}>{card.value}</strong>
              <span style={{ fontFamily: 'var(--cth-font-ui)', fontSize: 11, color: 'var(--cth-ink-700)' }}>{card.label}</span>
            </div>
            {card.agents.length > 0 && (
              <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 5 }}>
                {card.agents.slice(0, 3).map((agent) => (
                  <button key={agent.id} onClick={() => onSelect(agent.id)} style={{
                    border: 'none', cursor: 'pointer', padding: '1px 5px', background: `var(--cth-${agent.accent}-light)`,
                    color: 'var(--cth-ink-900)', fontFamily: 'var(--cth-font-ui)', fontSize: 11
                  }}>{agent.name}</button>
                ))}
              </div>
            )}
