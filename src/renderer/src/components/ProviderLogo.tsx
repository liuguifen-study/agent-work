...
export function ProviderLogo({ provider, size = 18, color = 'currentColor' }: ProviderLogoProps) {
  // A compact DSH monogram gives the Harness its own recognizable marker without
  // pretending to be DeepSeek's official whale logo.
  if (provider === 'deepseek') {
    return (
      <svg width={size} height={size} viewBox="0 0 24 24" role="img" aria-hidden="true" style={{ display: 'block', flexShrink: 0 }}>
        <rect x="2" y="2" width="20" height="20" rx="5" fill={color} />
        <text x="12" y="15.3" textAnchor="middle" fill="var(--cth-paper-100)" fontFamily="var(--cth-font-mono)" fontSize="8" fontWeight="700">DS</text>
      </svg>
    );
  }
