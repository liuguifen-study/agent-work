...
// (src/shared/hire.ts) — provider allowlist is claude | codex | antigravity | cursor | deepseek.
...
Rules:
- "provider" MUST be one of: cursor | claude | codex | antigravity | deepseek. "model" must be a real model id for that provider (e.g. gpt-5.6-luna-high, claude-opus-4-8[1m], gpt-5-codex, "Gemini 3.1 Pro (High)"). DeepSeek Harness selects its model in DSH settings, so omit "model" for it.
              ...
              <div style={{ flex: 1, minWidth: 0, minHeight: 260, display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {section === 'engine' && (
                  <>
                    ...
                    {provider === 'deepseek' && (
                      <div style={{
                        fontSize: 12, color: 'var(--cth-ink-700)', lineHeight: '17px', margin: '2px 0 6px',
                        padding: '7px 9px', background: 'var(--cth-sky-light)', boxShadow: 'inset 0 0 0 1px var(--cth-ink-300)'
                      }}>
                        {tr('addAgent.deepseekHarnessNote')}
                      </div>
                    )}
